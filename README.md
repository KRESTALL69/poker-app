...
**Автоматически:** каждый push в `main` запускает
`.github/workflows/deploy.yml` (по образцу ReRaise) — install → typecheck →
lint → test → sanity build, затем по SSH на VPS: production-сборка образа,
применение миграций (идемпотентно), пересоздание только контейнера `app`,
проверка running+healthy, smoke-test `/api/health` и `/`. Если что-то из
этого падает — деплой завершается неуспешно, а не тихо. Подробности,
список требуемых GitHub Secrets и откат — `docs/VPS_DEPLOYMENT.md`, раздел
«Автоматический деплой (CI/CD)».

**Вручную (fallback, если нужно или CI недоступен):**
...
Или через `scripts/deploy.sh`. nginx/DNS/TLS настраиваются один раз (см.
`deploy/nginx/dontworryclub.pro.conf`), обычный деплой их не трогает.
